			...
			switch ( $value['type'] ) {
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {
						...
						if (!is_numeric($select_ID))
							$theValue = $select_ID;
						$output .= '<option id="' . $select_ID . '" value="'.$select_ID.'" ' . selected($smof_data[$value['id']], $theValue, false) . ' />'.$option.'</option>';
					 } 
					$output .= '</select></div>';
				break;
				...
				case 'typography':
					...
					if(isset($typography_stored['face'])) {
                        ...
                        $faces = array('Lato' => '"Lato", sans-serif',
                            'arial'=>'Arial',
                            'GothamCondensedBold' => 'Ghotham Bold',
                            '"Yellowtail", cursive' => '"Yellowtail", cursive',
                            'GothamCondensedMedium' => 'Ghotham Medium',
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ',TEXTDOMAIN).'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
